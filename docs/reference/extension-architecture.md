...
6. `usage.js` normalizes the response through the selected provider's adapter from `usage-integration-adapters.js`, mapping adapter-declared weekly and five-hour paths first, then bounded path-matched candidates when the live usage shape is nested under adapter-recognized usage containers. Relative reset estimates are normalized to deterministic minute boundaries so equivalent polls compact; absolute upstream reset timestamps retain their supplied precision. Unrelated exact-duration quota-shaped objects are not accepted as supported windows.
