...
8. `background-refresh-runner.js` delegates the selected toolbar badge view and any critical-window attention override to `background-badge-presentation.js`, then writes refresh status through `refresh-status.js`; `background.js` orchestrates when the runner executes.
