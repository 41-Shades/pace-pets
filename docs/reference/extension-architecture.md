...
6. `usage.js` normalizes the response through the selected provider's adapter from `usage-integration-adapters.js`, mapping adapter-declared weekly and five-hour paths first, then bounded path-matched candidates when the live usage shape is nested under adapter-recognized usage containers. Positive relative reset estimates round up to deterministic minute boundaries so equivalent polls compact without moving a reset earlier than its full offset; absolute upstream reset timestamps retain their supplied precision. Unrelated exact-duration quota-shaped objects are not accepted as supported windows.
...
10. `dashboard.js` renders summaries, reset timing, and pace state from extension-local storage plus the page-scoped dashboard window selection, delegates chart rendering to the dashboard chart helper, then reuses cached state for minute-by-minute countdown and pace updates until storage changes or the page window selection changes. New dashboard pages seed from the stored badge window, but dashboard toggles stay in that page's `sessionStorage` and do not update the toolbar badge preference.
