...
- `collector/extension/background-transition-refresh.js` owns the adaptive
  ...
  less.
- `collector/extension/background-badge-presentation.js` owns toolbar badge
  rendering, stored badge-window selection, critical-window attention and local
  developer-preview selection, and empty-badge behavior. `background.js`
  orchestrates when that presentation boundary runs.
...
- `collector/extension/usage-windows.js` owns supported usage-window keys, durations, labels, the badge preference storage key, and latest-sample availability helpers shared by collection, storage, badge, and dashboard code.
...
- `collector/extension/audio-preferences.js` owns the persisted dashboard audio
  ...
  fade timing.
  `collector/extension/dashboard-brake-extreme-audio-timeline.js` owns the
  packaged single-clip Brake Extreme effects timeline, including its 3.75s
  duration, 0.85 volume, and 750ms final fade and default stop-fade timing.
...
6. `usage.js` normalizes the response through the selected provider's adapter from `usage-integration-adapters.js`, discovering adapter-declared paths first and then bounded path-matched candidates when the live usage shape is nested under adapter-recognized usage containers. Each recognized quota object is classified by its declared duration rather than its upstream `primary`, `secondary`, weekly, or five-hour path label: 300 minutes maps to the five-hour window and 10,080 minutes maps to the weekly window. Durationless and unsupported-duration objects are rejected instead of guessed, and unrelated exact-duration quota-shaped objects—including separate `additional_rate_limits` metered-feature buckets—are not accepted as supported account windows. The windows in the latest successful normalized sample are the account-specific availability source for dashboard and toolbar choices; a failed refresh leaves that sample unchanged, and no stored history defaults to the weekly view. Positive relative reset estimates round up to deterministic minute boundaries so equivalent polls compact without moving a reset earlier than its full offset; absolute upstream reset timestamps retain their supplied precision.
