...
const storageContractSources = Object.freeze([
  ...
  "collector/extension/refresh-status.js",
  "collector/extension/refresh-control.js",
  "collector/extension/themes/default/asset-manifest.js",
  "collector/extension/pace-state-art.js",
  "collector/extension/pace-state-special-data.js",
  "collector/extension/pace-state-data.js",
  "collector/extension/brake-intensity.js",
  "collector/extension/sprint-intensity.js",
  "collector/extension/developer-options.js",
  "collector/extension/dashboard-preferences.js",
  "collector/extension/storage-adapter.js",
  "collector/extension/audio-preferences.js",
  "collector/extension/history-store.js",
]);
...
async function loadStorageContracts() {
  ...
  for (const source of storageContractSources) {
    await importProjectScript(source);
  }

  return {
    dashboardPreferences: globalThis.PacePetsDashboardPreferences,
    audioPreferences: globalThis.PacePetsAudioPreferences,
    ...
    refreshControl: globalThis.PacePetsRefreshControl,
...
function developerBrakeIntensityPreviewExample({ developerOptions }) {
  return developerOptions.storedDeveloperOptionsValue({
    forcedPaceStateKey: "criticalBehind",
    brakeIntensityPreview:
      developerOptions.BRAKE_INTENSITY_PREVIEW_OPTIONS[3]?.value,
  });
}
...
export async function storageSchemaGeneratedMarkdown() {
  ...
  const developerBrakeIntensityPreviewStorage =
    developerBrakeIntensityPreviewExample(contracts);
  ...
  return [
    ...
    "",
    fencedJson(developerBrakeIntensityPreviewStorage),
    "",
    "### Runtime Constants",
    "",
    `- Storage key: \`${historyStore.HISTORY_STORAGE_KEY}\``,
    `- Refresh status key: \`${historyStore.REFRESH_STATUS_STORAGE_KEY}\``,
    `- Manual refresh cooldown key: \`${refreshControl.MANUAL_REFRESH_COOLDOWN_STORAGE_KEY}\``,
    `- Manual refresh cooldown: \`${refreshControl.MANUAL_REFRESH_COOLDOWN_MS / 1000}\` seconds`,
    ...
    `- Developer Brake hard intensity preview field: \`${developerOptions.BRAKE_INTENSITY_PREVIEW_KEY}\`; supported values: ${codeValueList(developerOptions.BRAKE_INTENSITY_PREVIEW_VALUES)}; stored only with \`${developerOptions.FORCED_PACE_STATE_KEY}: "criticalBehind"\``,
    ...
    `- Dashboard audio preference key: \`${audioPreferences.AUDIO_PREFERENCE_STORAGE_KEY}\``,
    `- Default dashboard audio enabled: \`${audioPreferences.DEFAULT_AUDIO_ENABLED}\``,
    `- Default dashboard audio volume: \`${audioPreferences.DEFAULT_AUDIO_VOLUME}\``,
    `- Dashboard audio volume range: \`${audioPreferences.MIN_AUDIO_VOLUME}\` to \`${audioPreferences.MAX_AUDIO_VOLUME}\``,
