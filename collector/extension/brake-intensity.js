(() => {
  ...
  const RATIO_RANGE = Object.freeze([0.55, 0]);
  const PREVIEW_RATIOS = Object.freeze([0.55, 0.45, 0.35, 0.25, 0.15, 0.05, 0]);
  ...
  function ratioForIntensity(value) {
    ...
    return floorRatio - (floorRatio - capRatio) * intensity;
  }
  ...
  function intensityForRatio(paceRatio) {
    ...
    const [floorRatio, capRatio] = RATIO_RANGE;
    const ratioSpan = floorRatio - capRatio;
    if (ratioSpan <= 0) {
      return 0;
    }

    return Math.max(0, Math.min(1, (floorRatio - numericRatio) / ratioSpan));
  }
  ...
  function previewOption(ratio) {
    ...
    return Object.freeze({
      ...
      ratio,
      status: `Brake hard intensity preview selected: ${formattedRatio}.`,
  ...
  globalThis.PacePetsBrakeIntensity = Object.freeze({
    PREVIEW_OPTIONS,
