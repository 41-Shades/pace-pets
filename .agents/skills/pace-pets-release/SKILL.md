...
5. Fetch, switch to local `main`, and fast-forward it to `origin/main`. Confirm any just-merged PR commit is contained in `HEAD` and the tree remains clean.
6. For a new release, compare the release source with the prior shipped tag. Require a runtime, packaged asset, permission, or other release-content change beyond the canonical version files; docs, release guardrails, listing copy, and version-only changes do not justify a Store package. Skip this gate when resuming a matching release or submission.
